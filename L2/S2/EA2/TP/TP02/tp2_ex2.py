...
def puissance_matrice_2_2 (M, n) :
    ...
    while n > 0 :
        if n % 2 == 1 :
            res = produit_matrice_2_2(res, M)
        M = produit_matrice_2_2(M, M)
        n = n // 2
    return res
...
# On remarque que fibo_3 devient plus lente tres rapidement, tandis que
# fibo_4 suitt la courbe de x**log2(3), ce qui lui donne une complexite
# de O(log n), ce qui est bien mieux que fibo_3 
...
def puissance_matrice_2_2_ops (M, n) :
    ...
    while n > 0:
        if n % 2 == 1:
            prod, new_ops = produit_matrice_2_2_ops(res, M)
            res = prod
            ops += new_ops
        prod, new_ops = produit_matrice_2_2_ops(M, M)
        M = prod
        ops += new_ops
        n //= 2
    return res, ops
...
def fibo_4_ops(n) :
    ...
    M = [ [1, 1], [1, 0] ]
    res, ops = puissance_matrice_2_2_ops(M, n - 1)
    return (res[0][0], ops)
...
def decompte_naif (a, b, c, d, e) :
    """
    Cout de l'operation a = b*c + d*e avec multiplication naive
    Pour la multiplication naive:
    - Une multiplication de nombres sur n bits sa coute n^2 opérations
    - Une addition de nombres sur n bits sa coute n opérations
    """
    ...
    cout_mult1 = nbbits_b * nbbits_c  # b*c
    cout_mult2 = nbbits_d * nbbits_e  # d*e
    
    taille_bc = nbbits_b + nbbits_c   
    taille_de = nbbits_d + nbbits_e 
 
    cout_add = max(taille_bc, taille_de)
    
    return cout_mult1 + cout_mult2 + cout_add
...
def decompte_karatsuba (a, b, c, d, e) :
  # cout de l'opération a = b*c+d*e lorsque la multiplication d'entiers utilise l'algorithme de Karatsuba
    ...
    cout_mult1 = int(max(nbbits_b, nbbits_c) ** 1.58)  # b*c
    cout_mult2 = int(max(nbbits_d, nbbits_e) ** 1.58)  # d*e
...
def produit_matrice_2_2_bits (M1, M2, compte = decompte_naif) :
    ...
    bits += compte(res[0][1], M1[0][0], M2[0][1], M1[0][1], M2[1][1]) 
    bits += compte(res[1][0], M1[1][0], M2[0][0], M1[1][1], M2[1][0])
    bits += compte(res[1][1], M1[1][0], M2[0][1], M1[1][1], M2[1][1])
...
def puissance_matrice_2_2_bits (M, n, compte = decompte_naif) :
   ...
   while n > 0:
       if n % 2 == 1:
           prod, new_bits = produit_matrice_2_2_bits(res, M, compte)
           res = prod
           bits += new_bits
       
       if n > 1:
           prod, new_bits = produit_matrice_2_2_bits(M, M, compte)
           M = prod
           bits += new_bits
       n //= 2
...
def fibo_4_bits(n, compte = decompte_naif) :
  ...
  M, bits = puissance_matrice_2_2_bits(M, n - 1, compte)
