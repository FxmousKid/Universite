...
# On remarque que fibo_1 a une complexite qui semble etre de de l'ordre x * phi^x
# et aussi que fibo_3 semble avoir une complexite de x^2, vu que leurs courbe on l'air
# de se chevaucher.
...
# O(log i)
def nbOfBits(i):
    # Sympa mais trop lent sur fibo_2_bits(n) pour n >= 20 000
    # i.bit_length() est plus rapide vu qu'elle est optimiser pour la representation
    # interne des large integers en python.
    # count: int = 0
    # while (i != 0):
    #     i >>= 1
    #     count += 1
...
def fibo_1_bits(n):
    ...
    t1 = fibo_1_bits(n - 1)
    t2 = fibo_1_bits(n - 2)
    return t1[0] + t2[0], t1[1] + t2[1] + nbOfBits(t1[0] + t2[0])
...
# Oui les courbes refletent bien les conclusions du cour
